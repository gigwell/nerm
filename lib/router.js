...
function parseOptions(req, res, next) {
  if(!!req.query.resEnvelope)
    req.resEnvelope = JSON.parse(req.query.resEnvelope)
  else {
    _.defaults(req.body, {resEnvelope: true})
    req.resEnvelope = req.body.resEnvelope
  }
...
exports.route = function(router, model, options) {
  ...
  router.get(manyUrl, function(req, res) {
    var query = Parser.parse(req.query.q || "{}")
    model.find(query)
      .exec(sendResults(pluralName, res, req.resEnvelope))
  })

  router.get(singleUrl, function(req, res) {
    model.findById(req.params.id)
      .exec(sendResults(singularName, res, req.resEnvelope))
  })
  ...
  router.post(manyUrl, function(req, res) {
    ...
    if (_.isArray(req.body[name])) {
      async.map(req.body[name], function(i, next) {
        ...
        item.save(next)
      }, sendResults(name, res, req.resEnvelope))
    } else {
      var item = new model(req.body[name])
      item.save(sendResults(name, res, req.resEnvelope))
    }
  })
  ...
  router.put(singleUrl, function(req, res) {
    model.findById(req.params.id).exec(errHandler(res, function(doc) {
      ...
      doc.save(sendResults(singularName, res, req.resEnvelope))
    }))
  })
